...
import { Play, Pause, RotateCcw, Coffee, Brain, Armchair, Settings2 } from "lucide-react"
...
export default function PomodoroPage() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] space-y-8 animate-in fade-in duration-500 relative">
      ...
      <div className="text-center space-y-2 relative w-full max-w-sm">
        ...
        {/* TOMBOL SETTINGS (POSISI POJOK KANAN) */}
        <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
          <DialogTrigger asChild>
            <Button variant="ghost" size="icon" className="absolute -top-1 right-0 text-zinc-400 hover:text-black">
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <form onSubmit={handleSaveSettings} className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="work" className="text-right">Fokus</Label>
                <Input id="work" name="work" type="number" defaultValue={durations.work} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="shortBreak" className="text-right">Rehat Kecil</Label>
                <Input id="shortBreak" name="shortBreak" type="number" defaultValue={durations.shortBreak} className="col-span-3" />
              </div>
              <div className="grid grid-cols-4 items-center gap-4">
                <Label htmlFor="longBreak" className="text-right">Rehat Besar</Label>
                <Input id="longBreak" name="longBreak" type="number" defaultValue={durations.longBreak} className="col-span-3" />
              </div>
              <DialogFooter>
      ...
      <div className="flex p-1 bg-zinc-100 rounded-lg border border-zinc-200">
        {(Object.keys(MODE_VISUALS) as Array<keyof typeof MODE_VISUALS>).map((key) => (
          <button
            key={key}
            ...
            className={cn(
              ...
              mode === key 
                ? "bg-white text-black shadow-sm ring-1 pring-black/5" 
      ...
      {/* === KARTU TIMER UTAMA === */}
      <div className={cn(
        "relative w-full max-w-sm p-8 rounded-2xl transition-colors duration-300 flex flex-col items-center justify-center gap-6",
        MODE_VISUALS[mode]
      )}>
        
        <div className={cn("p-3 rounded-full border-2 border-current bg-white", 
           mode === 'work' ? 'text-red-600' : mode === 'shortBreak' ? 'text-green-600' : 'text-blue-600'
        )}>
          <CurrentIcon className="w-8 h-8" />
        </div>

        <div className="text-8xl font-black tracking-tighter tabular-nums text-zinc-900">
          {formatTime(timeLeft)}
        </div>
        ...
        <div className="flex items-center gap-4">
          <Button
            ...
            className={cn(
              "h-14 px-8 rounded-full border-2 border-black text-lg font-bold transition-all hover:translate-y-1",
              isActive 
                ? "bg-white text-black hover:bg-zinc-100 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" 
                : "bg-black text-white hover:bg-zinc-800 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]"
            )}
          >
            {isActive ? (
              <>
          ...
          <Button
            ...
            size="icon"
            onClick={resetTimer}
            className="h-14 w-14 rounded-full border-2 border-black bg-white hover:bg-zinc-100 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1"
          >
